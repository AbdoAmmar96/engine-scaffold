...
└── tests/
    ├── Feature/                           18 ملف · 236 اختبار
    └── e2e/smoke.spec.ts                  Playwright — ديسكتوب وموبايل
```

**الأرقام:** 11 موديول · 138 راوت · 31 جدول · 65 ملف TSX · 12 صلاحية · 10 أدوار.
...
- **التقييمات حقيقية.** العميل بيكتب رأيه من حسابه، و`source` بيفرّقه عن اللي الأدمن كتبه، ومفيش رأي بينزل قبل الاعتماد. مفيش سيدر بيزرع آراء والقسم بيختفي وهو فاضي — ومفيش `AggregateRating` markup لأن جوجل بيمنع تقييم الموقع لنفسه.
- **«أضف عقارك» بيدّي حساب.** الزائر اللي يعرض وحدة بياخد حساب `lister` تلقائيًا ويوصله لينك يحدّد بيه كلمة سره — عشان يتابع وحدته وتوصله الطلبات فعلًا. قبل كده الوحدة كانت يتيمة (`owner_id = null`) والصفحة توعده بمتابعة مستحيلة.
