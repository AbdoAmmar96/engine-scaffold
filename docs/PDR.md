...
**لسه مفتوح:** مفيش CI (`.github/` لسه مش موجود) · راوتات `api/v1/*` لسه ورا guard `sanctum`
مش مركّب (`config('auth.guards')` = `["web"]`) · محتوى «من نحن» فيه صور وأسماء فريق مُختلَقة ·
الجدولة محتاجة cron على الإنتاج.
