...
| الجدولة محتاجة cron على الإنتاج | ⚠️ الكود جاهز (`cron.sh` + نبضة + تحذير في اللوحة) — **فاضل سطر واحد من hPanel** |

**لسه مفتوح:** مفيش CI (`.github/` لسه مش موجود) · محتوى «من نحن» فيه صور وأسماء فريق
مُختلَقة والأرقام مخترعة · موديولا `Pages` و`Reviews` لسه هياكل فاضية · `AdminLayout`
مش responsive (سايدبار `ps-64` ثابت — كل صفحات اللوحة بتطفح أفقيًا تحت 1024px).
