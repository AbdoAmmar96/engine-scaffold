...
## ⚠ تحديث الحالة — 2026-08-25

**المستند ده لقطة بتاريخ 2026-08-20، والأرقام اللي فيه اتخطّاها الشغل اللي بعده.**
سيبناه زي ما هو كمرجع للمراجعة الأصلية — بس متعتمدش على نسبه ولا على قايمة الفجوات.

اللي اتقفل من الفجوات الحرجة اللي المستند رصدها:

| اللي كان مرصود ناقص | الحالة دلوقتي |
|---|---|
| مفيش صفحة تفاصيل لعقار أو كمبوند | ✅ `/properties/{slug}` و `/compounds/{slug}` بمعرض وكود مرجعي |
| البحث decoration مش وظيفة | ✅ 23 فلتر و6 ترتيبات، كلها في الرابط |
| السعر مخزّن نص فمفيش فلترة رقمية | ✅ `price_amount` رقمي — والنص بقى override للعرض بس |
| مفيش `<Head>` ولا ميتا لأي صفحة | ✅ ميتا + canonical + hreflang + JSON-LD بتترندر في السيرفر |
| مفيش رفع ملفات خالص | ✅ مكتبة ميديا + رفع صور من «أضف عقارك» و«وحداتي» |
| مفيش `exists` validation على أي FK | ✅ متطبّقة في كل الريسورسات |
| الأدوار على الورق بس (`editor`/`agent` بياخدوا 403) | ✅ 10 أدوار و12 صلاحية مفروضة على الراوت وعلى الصف |
| PHPUnit 1 failed / 1 passed · مجلدات تستات فاضية | ✅ 187 اختبار Feature ناجحة |
| phpstan 78 خطأ | ✅ نزلت لـ 58 |
| مفيش deploy config | ✅ `deploy.sh` شغّال على استضافة مشتركة |

**اتقفل كمان (تحديث 2026-08-23):**

| اللي كان مرصود | الحالة دلوقتي |
|---|---|
| 🟠 CG-5 — 40 راوت API ميت (guard مش موجود) | ✅ اتشالوا (49 راوت) بدل ما يتحطّلهم جارد — راجع «مفيش طبقة API» في `CLAUDE.md` |
| الجدولة محتاجة cron على الإنتاج | ✅ اتضاف من hPanel ومتحقَّق منه — نبضة كل دقيقة + تحذير في اللوحة لو وقف |
| 🟠 CG-8 — محتوى «من نحن» مُختلَق (صور ستوك بأسماء متلفّقة وأرقام مخترعة) | ✅ اتشال بالكامل — كل نص فيها بقى من إعدادات `about` وفاضي افتراضيًا، والقسم الفاضي بيختفي |
| موديول `Pages` هيكل فاضي | ✅ اتبنى — صفحات محتوى على `/{locale}/{slug}` بـ CRUD وسيو وخريطة موقع |
| موديول `Reviews` هيكل فاضي | ✅ اتبنى — العميل بيقيّم من حسابه، الأدمن بيعتمد، والقسم بيختفي وهو فاضي |
| 🟡 CG-10 — مفيش CI | ✅ اتضاف — ريبو التطبيق فيه 5 بوابات على كل PR، والسكافولد بيتفحص بتشغيل `setup.sh` كامل |
| اللوحة مش responsive | ✅ اتظبطت — درج بينفتح على الموبايل، و12 شاشة اتقاسوا على 390 بكسل بصفر طفح |

**لسه مفتوح:**
باقي المحتوى تجريبي (١٣ وحدة و٦ مقالات) و`contact.whatsapp` فاضي · صفحتا الخصوصية
والشروط مسوّدات محتاجة مراجعة قانونية.

**المرجع المحدَّث:** `README.md` في جذر الريبو.
