...
- **`Pages`** — صفحات المحتوى الثابت (`/{locale}/{slug}`) + صفحة «من نحن».
- **`Reviews`** — آراء العملاء: العميل بيكتب رأيه من حسابه، والأدمن بيعتمده قبل ما ينزل.

الـ CRUD الأدمن كله بيرث من `app/Support/ResourceController.php` — أي تعديل هناك بيمسّ كل الموديولات.

---

## الراوتات (المصدر: `php artisan route:list --except-vendor` — 138 راوت)
...
**البوابات الأربعة خضرا، والـ CI بيحكم بيهم.** أي واحدة تحمرّ = الفرع مش بيتدمج.

- `pint --test` → **نضيف ✅**
- `phpstan level 5` → **نضيف ✅** — الـ 37 خطأ القديمة متجمّدة في `phpstan-baseline.neon`
- `tsc --noEmit` → **نضيف ✅**
- `php artisan test` → **236/236 ناجحة ✅** (18 ملف Feature)
- `playwright test` → **125 ناجحة + 1 متخطّى عن قصد ✅**

⚠️ **متزوّدش على الـ baseline.** لو PHPStan اشتكى من كودك، صلّحه — إعادة توليد
الـ baseline بتخفي الخطأ الجديد مع القديم وتفضي البوابة من معناها.

⚠️ `pint --dirty` بيفحص المتغيّر عن git بس. لو شغلك اتعمله commit، بيرجّع «نضيف»
من غير ما يفحص حاجة — استخدم `pint --test` أو عدّد الملفات صراحةً.

---

## الـ CI

- `engine/.github/workflows/ci.yml` — بيشتغل على كل push وPR: Pint · PHPStan ·
  TypeScript · PHPUnit · Playwright. الفحوص الساكنة بـ `fail-fast: false` عشان
  تشوف كل اللي واقع مش أول واحد.
- `.github/workflows/scaffold.yml` (جذر السكافولد) — بيشغّل `setup.sh` كامل
  ويطلب ٩ صفحات من المشروع المتولّد. **مش بيتشغّل على كل push** — أسبوعيًا
  ولما `setup.sh` أو `overlay/` يتغيّروا.

  الفحص ده مش نظري: أول تشغيل ليه كشف إن `sync-overlay.sh` ماكانش بينسخ
  `database/migrations` خالص، يعني **كل تثبيت جديد كان بيطلّع جدول `users`
  ناقص أعمدة** والسيدر بيقع على `is_active`. العطل ده عاش شهور من غير ما
  يبان، لأن `engine/` شغّال والسكافولد محدش بيشغّله.
- `migrate` لوحده بيغطّي ميجريشنز الموديولات — `module:migrate` من غير اسم موديول
  بيطلبه تفاعليًا وبيقع في الـ CI.
...
- **الزائر بياخد حساب.** `/add-property` مفتوح للكل، والإيميل **مطلوب** لأنه هو اللي
  بيحدّد الحساب: موجود → الوحدة بتتربط بيه ويوصله «وحدة جديدة على حسابك»، مش موجود →
  حساب `lister` جديد بكلمة سر عشوائية + رسالة «حدّد كلمة السر». قبل كده كانت بتتسجّل
  بـ `owner_id = null` والصفحة توعد بمتابعة مستحيلة. الرسالة الراجعة للفورم **واحدة في
  الحالتين** عشان مايبقاش أداة يعرف بيها حد مين عنده حساب. راجع `AddPropertyController::accountFor`.
...
- **آراء العملاء.** رأي واحد للحساب، `source` بيفرّق بين اللي العميل كتبه من حسابه
  واللي الأدمن كتبه بالإيد. مفيش رأي بيظهر قبل `status = published`، **وأي تعديل من
  العميل بيرجّعه للمراجعة** — نص اتعتمد ممنوع يتغيّر على الموقع من ورا الأدمن. مفيش
  سيدر بيزرع آراء والقسم بيختفي وهو فاضي: التقييمات أكتر محتوى بيتزوّر في المجال ده.
  **ومفيش `AggregateRating` في الـ JSON-LD** — جوجل بيمنع ماركب تقييمات الموقع عن
  نفسه، والمخالفة عقوبتها إجراء يدوي.
- **صفحات المحتوى تحت الجذر.** `/{locale}/{slug}` بيخدم صفحات `Pages` — رابط نضيف
  من غير بادئة. الراوت ده **لازم يفضل آخر حاجة** في مجموعة اللغة في `routes/web.php`
  (لارافيل بيطابق بترتيب التسجيل)، و`App\Support\ReservedSlugs` بيمنع أي صفحة تاخد اسم
  مسار موجود — **القايمة مشتقّة من الراوتر نفسه** عشان ما تتأخّرش عنه. صفحة اسمها
  «اتصل بنا» بتاخد `contact-2` تلقائيًا مش `contact`. `RouteHealthTest` بيحرس ده.
- **«من نحن» من الإعدادات.** كل نص فيها في مجموعة `about`، وكله **فاضي افتراضيًا**:
  القسم اللي مالوش بيانات بيختفي بدل ما يتلفّق. الصفحة كانت بتدّعي «اثنتا عشرة سنة»
  و«فريق من 46 شخص» وتعرض صور ستوك بأسماء متلفّقة — متردّش أي نص افتراضي فيه رقم أو ادعاء.
